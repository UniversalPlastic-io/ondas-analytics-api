...
/**
 * Fills Mongo from the data space bucket.
 *
 ...
 *
 *   npm run backfill               # whole root prefix
 *   npm run backfill -- --dry-run
 *   npm run backfill -- --force --prefix public/mediterraneo/
 */
async function main() {
  ...
  for (const r of summary.results) {
    ...
    console.log(`${r.action.padEnd(10)} ${r.key}${detail}`);
  }
  console.log('');
