 ...
 */
const OBSERVED_ONLY = { excludeProvider: REFERENCE_PROVIDER_FOLDER } as const;
...
export function bucketAverage(daily: Map<string, number>, period: OverviewPeriod): SeriesPoint[] {
  const groups = new Map<string, { sortKey: string; label: string; sum: number; n: number }>();
  for (const [date, value] of daily) {
    ...
    if (period === 'month') {
      ...
      sortKey = date.slice(0, 4); label = date.slice(0, 4);
    }
    const g = groups.get(sortKey);
    if (g) { g.sum += value; g.n += 1; }
...
export interface BiomassSection { meanTonnes: number; units: 'Tonnes'; readings: number; series: SeriesPoint[] }
...
export class OverviewSources {
  ...
  async loadBiomass(loc: { lat: number; lon: number }, period: OverviewPeriod): Promise<BiomassSection | null> {
    const asset = await this.assets.nearest({ ...OBSERVED_ONLY, category: 'biomass' }, loc);
    if (!asset) return null;
    const summary = asset.summary as { meanTonnes?: number; readings?: number };
    const daily = await this.observations.dailyMean({ assetIds: [asset._id] }, 'biomass_t_total');
  ...
  async loadMicroplastics(loc: { lat: number; lon: number }): Promise<MicroplasticsSection | null> {
    const asset = await this.assets.nearest({ ...OBSERVED_ONLY, category: 'microplastics' }, loc);
    if (!asset) return null;
  ...
  async loadEnvironment(loc: { lat: number; lon: number }): Promise<EnvironmentSection | null> {
    const asset = await this.assets.nearest({ ...OBSERVED_ONLY, category: 'environmental' }, loc);
    if (!asset) return null;
