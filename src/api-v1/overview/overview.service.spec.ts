...
const ROW = (date: string, kg: number): CleanupObservationRow => ({
  ...
  evidence: 3,
  polymers: { pct_pet: 40, pct_hdpe: 20, pct_ldpe: 10, pct_pp: 10, pct_ps: 0, pct_pvc: 0, pct_others: 0 },
});
...
function service(rows: CleanupObservationRow[]) {
  ...
  return { svc: new OverviewService(assets, observations, sources), assets, observations, sources };
}

describe('bucketSeries', () => {
  const rows: CleanupRow[] = [
    { date: '2025-01-05', location: 'A', city: 'A', kg: 10, volunteers: 1, km: 1, duration: '', evidence: 0, status: 'verified' },
    { date: '2025-01-06', location: 'A', city: 'A', kg: 20, volunteers: 1, km: 1, duration: '', evidence: 0, status: 'verified' },
    { date: '2026-02-01', location: 'A', city: 'A', kg: 5, volunteers: 1, km: 1, duration: '', evidence: 0, status: 'verified' },
  ];

  it('sums per day, per month and per year', () => {
    expect(bucketSeries(rows, 'month')).toHaveLength(3);
    expect(bucketSeries(rows, 'year')).toEqual([{ label: 'Jan', kg: 30 }, { label: 'Feb', kg: 5 }]);
    expect(bucketSeries(rows, 'all')).toEqual([{ label: '2025', kg: 30 }, { label: '2026', kg: 5 }]);
  });
});
...
describe('OverviewService', () => {
  it('projects KPIs from the cleanup rows the repository returns', async () => {
    ...
    expect(res.topLocations[0]).toMatchObject({ name: 'Badalona', cleanups: 2 });
  });
  ...
  it('scopes the query by campaign and period', async () => {
    ...
    expect(assets.findByFragments).toHaveBeenCalledWith(
      ['recogidas_playas_barcelona'],
      expect.objectContaining({ category: 'cleanup' }),
    );
  ...
  it('attaches the optional sections only when they resolve', async () => {
    ...
    (sources.loadBiomass as jest.Mock).mockResolvedValue({ meanTonnes: 5, units: 'Tonnes', readings: 10, series: [] });
  ...
  it('survives a failing supporting source', async () => {
    const { svc, sources } = service([ROW('2025-11-07', 1)]);
    (sources.loadEnvironment as jest.Mock).mockRejectedValue(new Error('mongo down'));
    const res = await svc.get('all', 'all');
